        ...
        <div class="grid grid-cols-2 gap-x-8 gap-y-4 p-4 mt-8 max-w-xl mx-auto">
            ...
            <div>
                ...
                <select 
                    ...
                    <option value="consulta">Cita médica</option>
                    <option value="medicacion">Medicamento</option>
                    <option value="ejercicio">Hábito saludable</option>
...
const navigateToSectionHealth = async () => {
    if (!form.value.salud.titulo || !form.value.salud.tipo || !form.value.salud.prioridad) {
    ...
    try {
        ...
        const healthData = {
            title: form.value.salud.titulo,
            category: form.value.salud.tipo,
            priority: form.value.salud.prioridad,
            description: form.value.salud.descripcion,
            due_date: form.value.salud.fecha ? new Date(form.value.salud.fecha).toISOString() : null
        };
        
        await healthService.create(healthData);
        showSuccessModal.value = true;
    } catch (error) {
        errorMessage.value = error.detail || 'Error al crear registro de salud';
